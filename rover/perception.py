...
def color_thresh(img, rgb_thresh=((180, 255), (180, 255), (160, 255))):
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
    # where threshold was met
    above_thresh = (rgb_thresh[0][0] <= img[:,:,0]) & (img[:,:,0] <= rgb_thresh[0][1]) \
...
def perception_step(Rover):
    ...
    dist = 5
    robot_offset = 6
    destination = np.float32([[img.shape[1]/2 - dist, img.shape[0] - robot_offset], \
                    [img.shape[1]/2 + dist, img.shape[0] - robot_offset], \
                    [img.shape[1]/2 + dist, img.shape[0] - 2 * dist - robot_offset], \
                    [img.shape[1]/2 - dist, img.shape[0] - 2 * dist - robot_offset]])
    ...
    rock_xpix, rock_ypix = rover_coords(rock_threshold_img)
    navigable_xpix, navigable_ypix = rover_coords(navigable_threshold_img)

    # 6) Convert rover-centric pixel values to world coordinates
    obstacle_x_world, obstacle_y_world = pix_to_world(obstacle_xpix, obstacle_ypix, Rover.pos[0], Rover.pos[1], Rover.yaw, Rover.worldmap.shape[0], scale)
    ...
    navigable_x_world, navigable_y_world = pix_to_world(navigable_xpix, navigable_ypix, Rover.pos[0], Rover.pos[1], Rover.yaw, Rover.worldmap.shape[0], scale)
