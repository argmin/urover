...
def create_output_images(Rover):
    ...
    nav_warped_img = Rover.vision_image[:,:,2]
    ...
    cv2.putText(map_add,"Fidelity: "+str(fidelity)+'%', (0, 40), 
            cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    cv2.putText(map_add,"Rocks Detected: "+str(Rover.samples_found), (0, 55), 
            cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    ...
    print('Time:', str(np.round(Rover.total_time, 1)), '| Mapped:', str(perc_mapped), '| Fidelity: ', str(fidelity), '| Rocks Found:', str(Rover.samples_found))
