...
def decision_step_experimental(Rover):
    if Rover.nav_angles is not None:
        # Check for Rover.mode status
        if Rover.mode == 'pickup':
            ...
            if is_feature_found(Rover.r_angles, Rover.o_dists):
...
#            if Rover.r_angles is not None and len(Rover.r_angles) > 0 and not np.isnan(np.mean(Rover.r_angles)):
            if is_feature_found(Rover.r_angles, Rover.r_dists):
                Rover.steer = np.clip(np.mean(Rover.r_angles * 180/np.pi), -15, 15)
                Rover = brake_control(Rover, True)
                Rover.mode = 'pickup'
            elif len(Rover.nav_angles) >= Rover.stop_forward:
                if Rover.vel == 0 and Rover.throttle > 0 and Rover.brake == 0:
                    ...
                    Rover.steer_dir = 1 if Rover.steer > 0 else -1
            # If there's a lack of navigable terrain pixels then go to 'stop' mode
            elif is_feature_found(Rover.o_angles, Rover.o_dists) and np.sum(np.mean(Rover.o_dists)) < 5:
                Rover = brake_control(Rover, True)
                Rover.mode = 'stop'
            elif len(Rover.nav_angles) < Rover.stop_forward:
                Rover = brake_control(Rover, True)
...
def decision_step(Rover, etype):
    if etype == 'udacity':
        return decision_step_udacity(Rover)
    elif etype == 'exp': 
        return decision_step_experimental(Rover)
    else:
